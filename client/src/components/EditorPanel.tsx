...
const EditorPanel = ({selectedElement,onUpdate,onClose}:EditorPanelProps) => {
    ...
    const handleStyleChange=async(styleName:string,value:string)=>{
  ...
  return (
    ...
    border border-gray-200 p-4 z-50 animate-fade-in fade-in'>
        <div className='flex justify-between items-center mb-4'>
            ...
            <button onClick={onClose} className='p-1 hover:bg-gray-800'>
        ...
        <div className='space-y-4 text-black'>
            <div>
                ...
                <textarea value={values.text} onChange={(e)=>handleChange('text',e.target.value)}
                    className='w-full text-sm p-2 border rounded-md focus:ring-2 focus:ring-indigo-500
                    outline-none min-h-20'/>
            ...
            <div className='flex items-center gap-2 border border-gray-400 rounded-md p-1'>
                <input type='color' 
                value={values.styles.backgroundColor ==='rgba(0,0,0,0)'?'#fffffff':values.styles.backgroundColor } onChange={(e)=>handleStyleChange('backgroundColor',e.target.value)}
            ...
            <div className='flex items-center gap-2 border border-gray-400 rounded-md p-1'>
                <input type='color' 
                value={values.styles.color} onChange={(e)=>handleStyleChange('color',e.target.value)}
                    className='w-6 h-6 cursor-pointer '/>
