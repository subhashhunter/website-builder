...
import { ArrowBigDownDashIcon, EyeIcon, EyeOffIcon, FullscreenIcon, LaptopIcon, Loader2Icon, MessagesSquareIcon, SaveIcon, Smartphone, Tablet, XIcon } from 'lucide-react'
  ...
  return project? (
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      ...
      no-scrollbar'>
        {/* {left} */}
        <div>
          <img src='/favicon.svg' alt='logo' className='h-6 curosr-pointer'
          onClick={()=>navigate('/')}/>
          ...
          <div className='sm:hidden flex-1 flex justify-end'>
            {isMenuOpen ? 
            <MessagesSquareIcon onClick={()=>setIsMenuOpen(false)} className='size-6 cursor-pointer'/>:<XIcon 
        ...
        <div className='hidden sm:flex gap-2 bg-gray-900 p-1.5 rounded-md'>
          <Smartphone onClick={()=>setDevice('phone')} className={`size-6 p-1 rounded cursor-pointer
          ${device==='phone'?'bg-gray-700':" "}`}/>
        
        <Tablet onClick={()=>setDevice('tablet')} className={`size-6 p-1 rounded cursor-pointer
          ${device==='tablet'?'bg-gray-700':" "}`}/>
       
        <LaptopIcon onClick={()=>setDevice('desktop')} className={`size-6 p-1 rounded cursor-pointer
          ${device==='desktop'?'bg-gray-700':" "}`}/>
        </div>
        
        {/* {right} */}
        <div onClick={saveProject} className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
          <button disabled={isSaving} className='max-sm:hidden bg-gray-800
          hover:bg-gray-700 text-white px-3.5 py-1 flex items-center gap-2 
          rounded-sm transition-colors border border-gray-700'>
            {isSaving ? <Loader2Icon className='animate-spin' size={16}/>:
          <SaveIcon size={16}/>}  Save
          </button>
          
          <Link target='_blank' to={`/preview/${projectId}`} className='flex 
          items-center gap-2 px-4 py-1 roundeed sm:rounded-sm border
          ...
          <button onClick={togglePublish} className='bg-linear-to-br from-blue-700 to-blue-600
          hover:from-blue-600 hover:to-blue-500 text-white px-3.5 py-1 flex
          items:center gap-2 rounded sm:rounded-sm transition-colors  '>
