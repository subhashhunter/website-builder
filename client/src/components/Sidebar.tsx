...
const handleRollback=async(versionId:string)=>{
 ...
 useEffect(()=>{
    ...
    transition-all ${isMenuOpen ? 'max-sm-0 overflow-hidden':'w-full'}`}>
        <div className='flex flex-col h-full'>
            {/* {message-container} */}
            <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col
            gap-4'>
                {[...project.conversation,...project.versions]
                .sort((a,b)=>new Date(a.timestamp).getTime()- new Date(b.timestamp).getDate()).
                map((message)=>{
                        ...
                        return (
                            ...
                            ${isUser?"justify-end" :"justify-start"}`}>
                                ...
                                <div className={`max-w-[80%] p-2 px-4 rounded-2xl
                                    shadow-sm text-sm mt-5 leading-relaxed
                                    ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-nonde":
                       ...
                       return(
                        ...
                        rounded-xl bg-gray-800 text-gray-100 shadow-flex flex-col gap-2'>
                            <div className='text-xs font-medium'>
                                code updated <br/>
                                <span className='text-gray-500 text-xs font-stretch-normal'>{new Date(ver.timestamp).toLocaleDateString()}</span>
                            </div>
            ...
            <form onSubmit={handleRevisions} className='m-3 relative'>
                <div className='flex items-center gap-2'>
                    <textarea onChange={(e)=>setInput(e.target.value)} rows={4} placeholder='Describe your website or request 
                    ...
                    <button disabled={isGenerating || !input.trim()} className='absolute
                    bottom-2.5  right-2.5 rounded-full bg-linear-to-r from-indigo-500
                    to-indigo-600 hover:from-indigo-600 
                    hover:to-indigo-700 text-white transition-colors disabled:opacity-60
                    '>
                        {isGenerating ? <Loader2Icon className='size-7 p-1.5 animate-spin
                        text-white'/>:
